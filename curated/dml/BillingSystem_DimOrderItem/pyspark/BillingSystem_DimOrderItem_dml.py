...
# Generated on: 2025-08-26T08:43:06.091Z
...
spark = SparkSession.builder \
    .appName("BillingSystem_DimOrderItem_DML_Operations") \
    .config("spark.sql.adaptive.enabled", "true") \
    .getOrCreate()

# Read raw data
raw_df = spark.read.parquet("path/to/raw/BillingSystem_DimOrderItem")

# Read existing curated data
curated_df = spark.read.parquet("path/to/curated/BillingSystem_DimOrderItem")
...
if new_records.count() > 0:
    ...
    transformed_new = new_records.select(
        col("OrderItemID").alias("OrderItemID"),
        col("OrderID").alias("OrderID"),
        col("ProductID").alias("ProductID"),
        col("Quantity").alias("Quantity"),
        col("UnitPrice").alias("UnitPrice"),
        col("LineAmount").alias("LineAmount"),
    ...
    transformed_new.write.mode("append").parquet("path/to/curated/BillingSystem_DimOrderItem")
...
if existing_records.count() > 0:
    ...
    updated_records.write.mode("overwrite").parquet("path/to/curated/BillingSystem_DimOrderItem_updated")
...
if deleted_records.count() > 0:
    ...
    deleted_records.write.mode("append").parquet("path/to/curated/BillingSystem_DimOrderItem_deleted")
