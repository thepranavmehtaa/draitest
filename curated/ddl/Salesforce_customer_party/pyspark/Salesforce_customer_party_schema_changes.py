...
# Generated on: 2025-08-26T09:43:00.835Z
...
spark = SparkSession.builder \
    .appName("Salesforce_customer_party_Schema_Changes") \
...
def add_control_columns(df):
    ...
    df = df.withColumn("_source_system", lit("raw_Salesforce_customer_party"))
    df = df.withColumn("_record_status", lit("valid"))
    df = df.withColumn("_update_timestamp", current_timestamp())
    df = df.withColumn("_batch_id", lit("batch_1756201380835"))
...
if __name__ == "__main__":
    # Read existing data
    df = spark.read.parquet("path/to/curated/Salesforce_customer_party")
    
    # Apply schema changes
    df = add_control_columns(df)
    
    # Write updated schema
    df.write.mode("overwrite").parquet("path/to/curated/Salesforce_customer_party_updated")
