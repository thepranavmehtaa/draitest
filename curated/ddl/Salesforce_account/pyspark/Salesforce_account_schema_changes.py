...
# Generated on: 2025-08-26T09:18:51.799Z
...
spark = SparkSession.builder \
    .appName("Salesforce_account_Schema_Changes") \
...
def add_control_columns(df):
    ...
    df = df.withColumn("_source_system", lit("raw_Salesforce_account"))
    df = df.withColumn("_record_status", lit("valid"))
    df = df.withColumn("_update_timestamp", current_timestamp())
    df = df.withColumn("_batch_id", lit("batch_1756199931799"))
...
if __name__ == "__main__":
    # Read existing data
    df = spark.read.parquet("path/to/curated/Salesforce_account")
    
    # Apply schema changes
    df = add_control_columns(df)
    
    # Write updated schema
    df.write.mode("overwrite").parquet("path/to/curated/Salesforce_account_updated")
