...
# Generated on: 2025-08-26T09:11:15.896Z
...
spark = SparkSession.builder \
    .appName("BillingSystem_DimOrder_Schema_Changes") \
...
def add_control_columns(df):
    ...
    df = df.withColumn("_batch_id", lit("batch_1756199475896"))
...
if __name__ == "__main__":
    # Read existing data
    df = spark.read.parquet("path/to/curated/BillingSystem_DimOrder")
    
    # Apply schema changes
    df = add_control_columns(df)
    
    # Write updated schema
    df.write.mode("overwrite").parquet("path/to/curated/BillingSystem_DimOrder_updated")
